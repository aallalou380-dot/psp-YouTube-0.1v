...
static void drawThumbnail(int x, int y, int w, int h, int selected, const char *duration)
{
    ...
    drawText(x + w - 45, y + h - 19, duration, COLOR_TEXT);
...
static void drawHeader(int page)
{
    ...
    drawRect(0, 0, SCREEN_WIDTH, 39, COLOR_PANEL);
    drawRect(0, 38, SCREEN_WIDTH, 2, COLOR_RED);

    /* Compact YouTube-style brand area. */
    drawRect(14, 9, 25, 20, COLOR_RED);
    drawPlayTriangle(27, 19, 5, COLOR_TEXT);
    drawText(48, 12, "PSP YOUTUBE", COLOR_TEXT);

    /* Current page pill. */
    drawRect(350, 8, 112, 22, COLOR_SOFT);
    drawText(365, 15, title, COLOR_TEXT);
}

static void drawVideoCard(int index, int x, int y, int w, int h, int selected)
{
    /* Card body is deliberately separate from the thumbnail. */
    drawRect(x - 2, y - 2, w + 4, h + 48, COLOR_CARD);

    if (selected) {
        drawRect(x - 2, y - 2, w + 4, 3, COLOR_RED);
        drawRect(x - 2, y + h + 44, w + 4, 2, COLOR_RED);
    }

    drawThumbnail(x, y, w, h, selected, videos[index].duration);

    drawText(x + 2, y + h + 7, videos[index].title, COLOR_TEXT);
    drawText(x + 2, y + h + 21, videos[index].channel, COLOR_DIM);
}
...
static void drawHome(int selectedVideo)
{
    ...
    drawText(18, 48, "RECOMMENDED", COLOR_DIM);

    drawVideoCard(0, 18, 55, 211, 78, selectedVideo == 0);
    drawVideoCard(1, 251, 55, 211, 78, selectedVideo == 1);
    drawVideoCard(2, 18, 164, 211, 78, selectedVideo == 2);
    drawVideoCard(3, 251, 164, 211, 78, selectedVideo == 3);
}
...
static void drawTrending(int selectedVideo)
{
    ...
    drawText(18, 48, "POPULAR NOW", COLOR_DIM);

    drawVideoCard(4, 18, 55, 211, 78, selectedVideo == 4);
    drawVideoCard(5, 251, 55, 211, 78, selectedVideo == 5);
    drawVideoCard(1, 18, 164, 211, 78, selectedVideo == 1);
    drawVideoCard(2, 251, 164, 211, 78, selectedVideo == 2);
}
...
static void drawSidebar(int x, int selected)
{
    ...
    drawRect(x + 10, 10, 25, 20, COLOR_RED);
    drawPlayTriangle(x + 23, 20, 5, COLOR_TEXT);
    drawText(x + 44, 14, "YOUTUBE", COLOR_TEXT);

    drawText(x + 14, 47, "MENU", COLOR_DIM);

    for (i = 0; i < 5; i++) {
        int y = 67 + i * 37;

        if (i == selected) {
            drawRect(x + 7, y - 5, SIDEBAR_WIDTH - 17, 30, COLOR_RED);
            drawRect(x + 7, y - 5, 3, 30, COLOR_TEXT);
        } else {
            drawRect(x + 10, y + 7, 5, 5, COLOR_SOFT);
        }

        drawText(x + 22, y + 3, items[i],
                 i == selected ? COLOR_TEXT : COLOR_DIM);
    }

    drawRect(x + 10, 245, SIDEBAR_WIDTH - 20, 1, COLOR_BORDER);
    drawText(x + 18, 253, "L CLOSE", COLOR_DIM);
}
