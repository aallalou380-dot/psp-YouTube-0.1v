...
{
    drawRect(0, 0, SCREEN_WIDTH, 48, COLOR_PANEL);
    drawRect(0, 47, SCREEN_WIDTH, 1, COLOR_LINE);

    /* Real YouTube mark only: no PSP YouTube text in the header. */
    drawYoutubeLogo(16, 10, 38, 25);

    if (showSearch) {
        drawRoundedPanel(171, 8, 239, 32, COLOR_PANEL2);
        drawRoundedOutline(171, 8, 239, 32, COLOR_LINE);
        drawSearchIcon(188, 23, COLOR_MUTED);

        if (searchQuery[0])
            drawTextClipped(201, 29, 195, searchQuery, 0.50f, COLOR_TEXT);
        else
            drawTextClipped(201, 29, 195, "Search YouTube", 0.50f, COLOR_MUTED);

        drawRoundedPanel(416, 9, 47, 30, COLOR_BG2);
        drawText(431, 28, "R", 0.48f, COLOR_TEXT);
    }

    drawText(18, 67, pageTitle, 0.80f, COLOR_TEXT);
    drawRect(18, 74, 42, 2, COLOR_RED);
}
...
static void drawVideoCard(int index, int x, int y, int w, int h, int selected)
{
    ...
    drawThumbnail(x + 7, y + 7, 108, h - 14, selected, v->style, v->duration);

    drawTextClipped(x + 124, y + 25, w - 132, v->cardTitle1, 0.47f, COLOR_TEXT);
    drawTextClipped(x + 124, y + 42, w - 132, v->cardTitle2, 0.44f, COLOR_TEXT);
    drawTextClipped(x + 124, y + 61, w - 132, v->channel, 0.41f, COLOR_MUTED);
    drawTextClipped(x + 124, y + 77, w - 132, v->views, 0.39f, COLOR_DIM);

    drawRoundedPanel(x + w - 25, y + h - 25, 18, 18, COLOR_BG2);
    drawRect(x + w - 20, y + h - 20, 3, 3, COLOR_MUTED);
    drawRect(x + w - 20, y + h - 15, 3, 3, COLOR_MUTED);
    drawRect(x + w - 20, y + h - 10, 3, 3, COLOR_MUTED);

    drawCardSelection(x, y, w, h, selected);
}
...
static void drawShortCard(int index, int x, int y, int w, int h, int selected)
{
    ...
    drawRoundedPanel(x, y, w, h, COLOR_PANEL);
    drawThumbnail(x + 7, y + 7, w - 14, 92, selected, v->style, v->duration);
    drawTextClipped(x + 9, y + 119, w - 18, v->cardTitle1, 0.45f, COLOR_TEXT);
    drawTextClipped(x + 9, y + 136, w - 18, v->cardTitle2, 0.41f, COLOR_TEXT);
    drawTextClipped(x + 9, y + 153, w - 18, v->channel, 0.39f, COLOR_MUTED);
    drawTextClipped(x + 9, y + 169, w - 18, v->views, 0.37f, COLOR_DIM);
    drawCardSelection(x, y, w, h, selected);
}
...
static void drawHome(int selected)
{
    ...
    drawVideoCard(0, 16, 84, 220, 88, selected == 0);
    drawVideoCard(1, 244, 84, 220, 88, selected == 1);
    drawVideoCard(2, 16, 178, 220, 88, selected == 2);
    drawVideoCard(3, 244, 178, 220, 88, selected == 3);
}
...
static void drawTrending(int selected)
{
    ...
    drawVideoCard(4, 16, 84, 220, 88, selected == 0);
    drawVideoCard(5, 244, 84, 220, 88, selected == 1);
    drawVideoCard(1, 16, 178, 220, 88, selected == 2);
    drawVideoCard(3, 244, 178, 220, 88, selected == 3);
}
...
static void drawSearchPage(int selected)
{
    ...
    if (searchQuery[0] == '\0') {
        drawRoundedPanel(52, 106, 376, 82, COLOR_PANEL);
        drawSearchIcon(94, 142, COLOR_RED);
        drawText(122, 137, "Search YouTube", 0.66f, COLOR_TEXT);
        drawText(122, 161, "Press R to open the PSP OSK.", 0.47f, COLOR_MUTED);
        return;
    }

    drawText(18, 94, "RESULTS", 0.46f, COLOR_MUTED);
    drawVideoCard(0, 16, 104, 220, 88, selected == 0);
    drawVideoCard(1, 244, 104, 220, 88, selected == 1);
    drawVideoCard(4, 16, 178, 220, 88, selected == 2);
    drawVideoCard(5, 244, 178, 220, 88, selected == 3);
}
...
static void drawVideoPage(int selectedVideo, int playing)
{
    ...
    /* Player / preview field */
    drawRoundedPanel(18, 84, 281, 118, COLOR_PANEL2);
    drawRoundedOutline(18, 84, 281, 118, COLOR_LINE);
    drawThumbnailArt(22, 88, 273, 110, v->style);
    drawRect(22, 88, 273, 110, 0x58000000);

    playerColor = playing ? COLOR_GREEN : COLOR_RED;
    drawRoundedPanel(132, 126, 54, 38, playerColor);
    if (playing)
        drawPauseIcon(159, 145, COLOR_WHITE);
    else
        drawPlayIcon(159, 145, 8, COLOR_WHITE);

    drawText(119, 187, playing ? "PLAYING" : "READY", 0.44f, COLOR_WHITE);

    /* Action / metadata column on the upper right */
    drawRoundedPanel(309, 84, 153, 118, COLOR_PANEL);
    drawRoundedOutline(309, 84, 153, 118, COLOR_LINE);

    drawRoundedPanel(322, 94, 127, 32, COLOR_RED);
    drawPlayIcon(341, 110, 6, COLOR_WHITE);
    drawText(356, 116, "PLAY", 0.52f, COLOR_WHITE);

    drawText(322, 148, v->channel, 0.50f, COLOR_TEXT);
    drawText(322, 168, v->views, 0.45f, COLOR_MUTED);
    drawText(322, 188, v->duration, 0.43f, COLOR_DIM);

    /* Title + description box */
    drawRoundedPanel(18, 210, 444, 52, COLOR_PANEL);
    drawRoundedOutline(18, 210, 444, 52, COLOR_LINE);
    drawTextClipped(30, 229, 420, v->title, 0.56f, COLOR_TEXT);
    drawTextClipped(30, 248, 420, v->description, 0.41f, COLOR_MUTED);

    drawText(390, 67, "O BACK", 0.40f, COLOR_DIM);
}
